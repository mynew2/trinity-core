...
public:
    ...
    static bool HandleMainTankCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (!*args)
        {
            if (uint64 selection = handler->GetSession()->GetPlayer()->GetTarget())
            {
    ...
    static bool HandleNpcBotAddCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (owner->GetGroup())
        {
            handler->PSendSysMessage("组队中无法添加机器人");
            handler->SetSentErrorMessage(true);
            return false;
        }
        if (owner->isDead())
        ...
        else if (!strncmp(bclass, "qianxing", 6) || !strncmp(bclass, "rog", 4) || !strncmp(bclass, "ro", 3))
            botclass = CLASS_ROGUE;
        else if (!strncmp(bclass, "saman", 7) || !strncmp(bclass, "sha", 4) || !strncmp(bclass, "sh", 3))
